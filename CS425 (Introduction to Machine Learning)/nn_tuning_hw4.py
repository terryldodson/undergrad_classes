...
#   General Model               #
# # # # # # # # # # # # # # # # #
def DynamicModel(neurons=4, activation_func='sigmoid'):
    """ A sequential Keras model that has an input layer, one 
        hidden layer with a dymanic number of units, and an output layer."""
    model = Sequential()
    model.add(Dense(neurons, input_dim=4, activation=activation_func, name='input_layer'))
    model.add(Dense(neurons, activation=activation_func, name='hidden_layer_1'))
    model.add(Dense(neurons, activation=activation_func, name='hidden_layer_2'))
    #model.add(Dense(neurons, activation=activation_func, name='hidden_layer_3'))
    #model.add(Dense(neurons, activation=activation_func, name='hidden_layer_4'))
...
model = KerasClassifier(build_fn=DynamicModel, verbose=0)
